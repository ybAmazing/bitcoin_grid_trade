...
def getAccountInfo(method):
    ...
    while True:
        try:
            r = requests.post(HUOBI_SERVICE_API, params=payload, verify=False)
        except:
            pass
        else:
            break
...
def getCurrentMarket():
    while True:
        try:
            r = requests.get("http://api.huobi.com/staticmarket/ticker_btc_json.js", verify=False)
        except:
            pass
        else:
            break
